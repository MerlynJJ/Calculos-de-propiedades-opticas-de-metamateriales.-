...
localizado. La frecuencia de este movimiento en un sistema infinito se
denomina como
...
localizada en alguna región $\mathcal{R}$, ésta producirá un campo
...
$\bm{\mathcal{P}}=-n_0 e\bm \zeta$, como ilustra la fig. \ref{Polariton},
\begin{figure}
  \centering
...
superficie deben coicidir para las ondas reflejada,  transmitida e
...
\begin{equation}
  \label{Maxwell-eqs}
  \begin{array}{cc}
    \nabla \cdot {\bm D} = 4\pi \rho_{ex}, & \nabla \cdot {\bm B} =0, \\
    \nabla \times {\bm E} = -\frac{1}{c}\frac{\partial {\bm B}}{\partial t}, &
    \nabla \times {\bm H} = \frac{4\pi}{c}{\bm
      J}_{ex} + \frac{1}{c}\frac{\partial {\bm D}}{\partial t},
  \end{array}
\end{equation}
donde ${\bm J}_{ex}$ y $\rho_{ex}$ son la densidad de corriente y
...
\cite{haydock} para calcular proyeccionies de funciones de Green. De
...
\begin{equation}
  {\bm D}_{p} = \hat{\epsilon}_{M} {\bm E}_{p}
  \label{Def-Dilectric-Op}
\end{equation}
donde ${\bm D}_{p}$ y ${\bm E}_{p}$ representan los promedios macroscópicos del
campo desplazamiento y eléctrico, respectivamente.

Sin embargo es posible definir ${\bm D}$ y ${\bm H}$ para incluir
...
\begin{equation}
  {\bm D} = \hat{\epsilon} {\bm E},
  \label{MicroscopicDielectric-Op}
\end{equation}
la cual acopla las fluctuaciones locales de los campos ${\bm D}$ y
${\bm E}$ totales y a la cual se le aplica un procedimiento de
...
${\bm D}_{p}$ y ${\bm E}_{f}$ de la ecuación de campos completos
\eqref{CamposCompletos}. Usando las ecuaciones de Maxwell para
los campos microscópicos se encuentra una relación entre ${\bm E}_{p}$
y ${\bm E}_{f}$,
\begin{equation}
  \begin{split}
      \nabla \times \nabla \times {\bm E} = \frac{4\pi
        i\omega}{c^{2}}{\bm j}_{ex} + \frac{\omega^{2}}{c^{2}}{\bm D}
      \\ \left[ \hat{\epsilon}_{ff} - \frac{c^{2}}{\omega^{2}}(\nabla
        \times \nabla \times)_{ff}\right] {\bm E}_{f} = \frac{4\pi}{i
        \omega }{\bm j}_{f}^{ex}-\hat{\epsilon}_{fp}{\bm E}_{p}
  \end{split}
\end{equation}
asumiendo que ${\bm J}^{ex}_{f} = 0 $, sustituimos ${\bm E}_{f}$ en la
ecuación \eqref{CamposCompletos},
\begin{equation}
  {\bm
    D}_{p}=\left(\hat{\epsilon}_{pp}-(\hat{\epsilon}_{pf}(\hat{\epsilon}_{ff}-\frac{c^{2}}{\omega^{2}}(\nabla
  \times \nabla)_{ff})^{-1}\hat{\epsilon}_{fp}\right){\bm E}_{p},
...
\begin{equation}
  \hat{P}^{T}=-\hat{\nabla}\times \hat{\nabla}^{-2} \hat{\nabla}\times,
\end{equation}
tales que extraen las partes irrotacional ${\bm F}^{L}=\hat{P}^{L}{\bm
  F} $ y solenoidal ${\bm F}^{T}=\hat{P}^{T}{\bm F}$ de una campo
...
otros métodos relacioniados a la geometría del sistema, a partir del
...
\begin{figure}
  ...
  \caption{(Izquierda) Película de Ag  depositada en vidrio con una
    ...
    ancho con parámetros entonados para maximizar el dicroismo a una
...
Department of Electronics and Nanoengineering, School of
